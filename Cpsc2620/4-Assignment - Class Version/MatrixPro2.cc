...
CMatrixPro CMatrixPro::operator*(const CMatrixPro &M){
  ...
  CMatrixPro temp = CMatrixPro(dimension);
...
//multiply matrix by matrix need a work on
CMatrixPro& CMatrixPro::operator*=(const CMatrixPro &M){
  assert(M.dimension == dimension);
  CMatrixPro temp = CMatrixPro(dimension);
  for(int i = 0; i < dimension; i++) {
    for(int j = 0; j < dimension; j++) {
      int sum = 0;
      for(int k = 0; k < dimension; k++){
        //cout<<matrix[i][k] <<" * " << M.matrix[k][j]<< " = "<< sum<<endl;
...
//multiply constant to matrix elements
CMatrixPro& CMatrixPro::operator*=(int x){
  //CMatrixPro differnceMatrix = *this;
  for (int i = 0; i < this->dimension; i++) {
    for (int j = 0; j < this->dimension; j++) {
